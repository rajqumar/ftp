...
export function InvBuyView(props) {
  ...
  if (invoice !== undefined && invoice.invoice_view && Object.keys(invoice.invoice_view).length > 0) {
    ...
    if(status.buyer === 'Acknowledge') {
      setProgress('paid')
    } else if(status.buyer === 'Acknowledged') {
      setProgress('acknow')
    }
    ...
    var info = {
      ...
      payment: viewInvoice(`Upload Date: ${dateformat(payment_uploaded_at)}`, payment_receipt),
    };
    info_data.push(info);
    var exProducts = []
    ...
    const info_columns = [
      ...
      },
      {
        ...
        minWidth: '80px',
        sortable: true,
        omit: payment_receipt ? false : true,
      },
      {
        ...
        minWidth: '80px',
        sortable: true,
        omit: payment_receipt ? true : false,
        cell: (row, i) => (
          <React.Fragment>
          <Plain
            data={{
    ...
    dispatch(change('status', 'payment_sent'));

    if (submitting) {
      
      return <PageLoader text="Sending Payment ..." />;
    }
    var actionBtns = status.buyer === 'Acknowledge' || status.buyer === 'Payment ssent' || status.buyer === 'Acknowledged' ? true : false
// console.log('asdasfasf', status)
    return (
      <form encType="multipart/form-data" onSubmit={handleSubmit(invStatusActionHandler)}>

        <div className="row">
          <div className="col-md-6">
          <BackLink link="/invoice/buy/list" icon={faAngleDoubleLeft} value=" Back to PR-200210-12345" />
