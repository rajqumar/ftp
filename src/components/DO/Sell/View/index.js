...
export function DOSellView(props) {
  ...
  if (dol !== undefined && dol.do_view && Object.keys(dol.do_view).length > 0) {
    ...
    if (submitting) {
      ...
      return <PageLoader text="Approving Delivery Order ..." />;
    }
    ...
    var actionBtns = status.seller === 'DO Accepted' || status.seller === 'Pending Acceptance' ? true : false

    return (
      <form onSubmit={handleSubmit(doStatusActionHandler)}>
        <div className="row">
          <div className="col-md-6">
            <BackLink link="/do/sell/list" icon={faAngleDoubleLeft} value=" Back to PR-200210-12345" />
